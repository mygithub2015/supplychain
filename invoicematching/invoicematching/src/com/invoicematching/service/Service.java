...
import com.invoicematching.beans.PromotionDetails;
...
public class Service {
	...
	public void service(Promotion promotion){
		...
		dao.setTemplate(template);
		for(PromotionDetails promoDtls : promotion.getPromotionDetails()){
			promoDtls.setPromotionId(promotion.getPromotionId());
			promoDtls.setLocnId(101);
			promoDtls.setLocnAddr("Whitefield");
			promoDtls.setCity("Bangalore");
			promoDtls.setState("Karnataka");
			dao.save(promoDtls);
		}
		/*invoice.setTotalAmt(totalAmt);
		dao.save(invoice);*/
